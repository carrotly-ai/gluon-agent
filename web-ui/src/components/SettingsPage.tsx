...
export function SettingsPage({ tab: controlledTab, onTabChange }: SettingsPageProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setWorkspaces(ws)
      setProjects(prj)
      setLlmProvider((settings.llm_provider === 'anthropic' ? 'anthropic' : 'bedrock') as 'bedrock' | 'anthropic')
            ...
            <div className="p-4 bg-[rgba(163,163,163,0.04)] border border-[rgba(163,163,163,0.1)] rounded-sm space-y-4">
              ...
              <div>
                ...
                <p className="text-caption text-[var(--color-stone)]/70 mt-1">
                  ...
                  and billing. Anthropic (Direct) uses your Anthropic API key or Claude subscription.
                </p>
              </div>
              ...
              {llmProvider === 'anthropic' && (
                <p className="text-caption text-[var(--color-stone)]/60">
                  Requires <code className="text-[var(--color-paper)]/80">ANTHROPIC_API_KEY</code> env
                  var or <code className="text-[var(--color-paper)]/80">claude login</code>. New agent
                  runs will use Anthropic model IDs. Running agents are not affected.
                </p>
              )}
            </div>
