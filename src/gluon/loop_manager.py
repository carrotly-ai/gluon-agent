...
def autonomy_promotion_hint(loop: AgentLoop, metrics: dict) -> str | None:
    """A SUGGESTION (never automatic) that a loop has earned a higher autonomy
    tier, based on its own track record (loop-hardening Phase F6). Promotion is
    a human decision — this only surfaces eligibility so autonomy follows
    evidence, not a create-time guess. None when not eligible / already L3.
    """
    if loop.autonomy not in ("L1", "L2"):
        return None
    runs = int(metrics.get("runs", 0) or 0)
    if runs < 5:
        return None  # not enough evidence yet
    pr_producing = int(metrics.get("pr_producing", 0) or 0)
    accepted = int(metrics.get("accepted", 0) or 0)
    # Require a real acceptance record: PRs produced and (mostly) accepted.
    if pr_producing < 3 or accepted / pr_producing < 0.8:
        return None
    nxt = "L2" if loop.autonomy == "L1" else "L3"
    return (
        f"eligible for {nxt}: {runs} runs, {accepted}/{pr_producing} changes accepted "
        f"({accepted / pr_producing:.0%}) — promote if you trust its judgment"
    )
...
class LoopManager:
    ...
    def _pause(self, loop: AgentLoop, reason: str) -> None:
        ...
        self._write_state_projection(loop.id)
    ...
    def _write_state_projection(self, loop_id: str) -> None:
        """Repo-visible loop-state projection (loop-hardening Phase F7).

        Writes a generated, read-only ``.gluon/LOOP-STATE.md`` digest into the
        project so the loop's memory is inspectable from the repo without the
        Gluon UI. The DB stays authoritative; this file is a VIEW (inverse of the
        loop-engineering file-as-authority model). Opt-in: only written when a
        ``.gluon/`` directory already exists in the project. Never raises.
        """
        try:
            loop = self.store.get_agent_loop(loop_id)
            if loop is None:
            ...
            if project is None:
                return
            gluon_dir = project.expanded_path / ".gluon"
            if not gluon_dir.is_dir():
                return  # operator hasn't opted in
            recent = self.store.list_runs_for_loop(loop.id, limit=8)
            lines = [
                f"# LOOP-STATE — {loop.objective[:80]}",
                "",
                "_Generated by Gluon (read-only view; the database is authoritative)._",
                "",
                f"- **Status:** {loop.status.value}" + (f" — {loop.status_reason}" if loop.status_reason else ""),
                f"- **Autonomy:** {loop.autonomy}  ·  **Gate:** {loop.verify_cmd or 'gateless'}",
                ...
                f"- **Pending tasks:** {self.store.count_pending_loop_items(loop.id)}",
            ]
            if loop.last_verdict:
                lines.append(f"- **Last verifier verdict:** `{loop.last_verdict}`")
            lines += ["", "## Recent iterations", ""]
            for r in recent:
                cost = f"${r.cost_usd:.2f}" if r.cost_usd else "$0.00"
                lines.append(f"- `{r.id[:8]}` {r.status.value} {cost} — {(r.custom_title or r.prompt or '')[:70]}")
            (gluon_dir / "LOOP-STATE.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
        except Exception:
            logger.debug("Loop state projection write failed", exc_info=True)
