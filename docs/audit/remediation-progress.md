...
## NEXT STEPS
Items 1–6 done; item 7 assessed + deferred to #161 (candidate dup blocks drifted — needs a design call). Next: **item 8 — chat_agent tool cache** (§2): `chat()` rebuilds all 41 MCP tools + `create_sdk_mcp_server()` on every message — cache the tool set + server on `self`; derive `allowed_tools` from the decorated tool functions instead of the 3 hand-synced lists. chat_agent.py is not in the central list, but run the FULL suite anyway (chat tests). Behavior-preserving.
