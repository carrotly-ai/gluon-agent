...
## NEXT STEPS
Items 1–4 done. Next: **item 5 — drop store `in keys` guards** in `_row_to_run`/`update_run` (store.py §2): migrations are idempotent and always run before any read, so the `row["x"] if "x" in keys else default` guards (~70 LOC) are obsolete. store.py is CENTRAL → run the FULL backend suite. Then item 6 (web/api lookup helpers).
