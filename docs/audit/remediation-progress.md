...
- [~] **3. Leaf dead code (Clusters E–J)** `[med/low/very-low]` — re-grep each symbol before deletion; delete test-only symbols + their tests together.
  ...
  - [~] **3B — test-only symbols + their tests**:
    ...
    - [ ] **3B-ii**: `policies.should_auto_resume` (+ TestShouldAutoResume); `recurrence.cron_to_recurrence` (+ `_CRON_TO_ISO` now orphaned + scattered tests in test_task_schedules); `bot_core.resolve_project`/`get_task` (+ TestResolveProject + the get_task asserts in test_bot_core_helpers).
...
## NEXT STEPS
Items 1–2 done; item 3A done; item 3B-i done (image_storage + blueprint.run_validation, −17 tests). Next: **item 3B-ii** — `policies.should_auto_resume` (+ TestShouldAutoResume in test_policies.py), `recurrence.cron_to_recurrence` (+ orphaned `_CRON_TO_ISO` + the 4 scattered cron tests in test_task_schedules.py, keeping the recurrence_to_cron tests), `bot_core.resolve_project`/`get_task` (+ TestResolveProject + the 3 get_task asserts in test_bot_core_helpers.py). Then item 3C, then item 4.
