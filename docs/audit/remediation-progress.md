...
## NEXT STEPS
**Item 3 (leaf dead code) fully done** (3A/3B/3C). Next: **item 4 — web-ui dead exports** (~30 dead `api.ts` functions + 12 orphan `types.ts` types + `POLL_FAST` + stale TODOs, per §7; EXCLUDE the 13 advanced-git api.ts clients → DEFER/#157). Frontend gate: `cd web-ui && bunx @biomejs/biome check <files> && bun run build`. Re-grep each export across web-ui/src before deleting.
