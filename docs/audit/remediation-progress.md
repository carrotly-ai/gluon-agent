...
## NEXT STEPS
Items 1–6, 8, 9 done; item 7 deferred (#161). Next: **item 10 — budget enforcement core↔runner** (§4.6): `_enforce_agent_budget` (core.py ≡ runner.py) and `_enforce_workspace_budget` (core.py ≡ runner.py, incl. identical 80%-headroom warning strings) are verbatim dupes. GROUND FIRST (item-7 lesson): diff the core vs runner versions to confirm they're TRULY identical before extracting; if they diverge, defer. If identical, extract one canonical free-function (or shared mixin) and have both delegate — behavior-preserving. Both files central → FULL suite.
