...
## NEXT STEPS
Items 1–6, 8, 9, 10 done; item 7 deferred (#161). Next: **item 11 — dead formula routes** (§3J): add `tests/test_api_formulas.py` FIRST (cover the LIVE formula routes — list/run — to lock their behavior), THEN remove only the 2 internally-dead formula routes `POST /api/formulas/validate` + `GET /api/formulas/{name}` (re-grep web-ui to confirm 0 callers; api.ts `fetchFormula`/`validateFormula` were already removed in item 4). Do NOT touch provider/budget/supervision routes (DEFER/#159). web/api.py central → FULL suite.
