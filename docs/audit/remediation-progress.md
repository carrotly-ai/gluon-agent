...
## NEXT STEPS
Items 1–6, 8 done; item 7 deferred (#161). Next: **item 9 — git_manager `_run_gh` helper + dedupe** (§4.7): every `gh` call hand-rolls `create_subprocess_exec` + `communicate` + rc-check + an inline `import json as json_module` (while git calls go through `_run_git`). Add `_run_gh(cwd, *args, parse_json=False)` mirroring `_run_git`; also the `except (FileNotFoundError, Exception)` is redundant (`FileNotFoundError ⊂ Exception`). GROUND: read `_run_git` + a few `gh` call sites to match exact error handling; behavior-preserving; fold incrementally. git_manager.py is mypy-ignored; run FULL suite.
