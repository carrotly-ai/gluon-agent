...
- [ ] **[P2] ~15 `GluonStore` public methods have zero callers** — [src/gluon/store.py:3702](src/gluon/store.py#L3702) 🔎 verify-first
  ...
  - **Status:** PARTIALLY DONE. Of the 3 spot-checked: `get_run_by_thread_id` was removed and the two TTL sweepers are now wired, so they're no longer dead. Two further **zero-reference** public methods (no caller, no test anywhere) were removed: `get_session_with_project` and `delete_channel_mapping`. Methods with a test (`delete_all_workspace_settings`, `delete_old_notifications`, `update_project`) and the Worker/Job queue methods (`create_job`, `delete_worker`, `update_worker_heartbeat` — used by the documented RedisJobQueue subsystem) are deliberately kept. A blanket removal of any remaining suspected-dead methods is intentionally **not** done autonomously — `store` is a public API surface; some methods may be invoked dynamically. Left unchecked pending a per-method review with the owner.
...
- [x] **[P2] `interface AgentMessage` declared 4× and `parseMessages` copied 3×** — _DONE: extracted the canonical `AgentMessage` (the rich superset StreamingLogViewer relied on — typed `metadata` for rendering) plus the single `parseMessages` JSONL parser into `web-ui/src/lib/agentMessage.ts`. All four components import it; the three narrow copies + three identical `parseMessages` are gone. Reconciliation note: the list/detail views only read `type`/`content`/`timestamp` and feed `parseMessages(...)` into StreamingLogViewer's `initialMessages`, so widening them to the superset is a no-op (verified: `bun run build` type-checks clean). Distinct from `types.ts:AgentMessageData` (the over-the-wire WS payload with `Record<string, unknown>` metadata), documented inline._ — [web-ui/src/lib/agentMessage.ts](web-ui/src/lib/agentMessage.ts)
...
- [x] **[P2] `formatDuration`/`formatTokens` duplicated in 4 components; name collision with different output** — _DONE: the three `formatDuration` (seconds) and four `formatTokens` copies were byte-identical to `lib/format.ts` (only param type differed), so the components now import the canonical helpers. The collision is resolved by **renaming** StreamingLogViewer's local ms formatter `formatDurationMs` → `formatElapsedMs`: it renders sub-second precision (`523ms`/`5.2s`/`2m3s`) and intentionally differs from `format.ts:formatDurationMs` (`formatDuration(ms/1000)`), so it was NOT centralized — a blind merge would have changed displayed values. Documented inline. `bun run build` type-checks clean._ — [web-ui/src/lib/format.ts](web-ui/src/lib/format.ts)
...
- [x] **[P2] `is_authorized` duplicated (telegram vs bot_core shared)** — _DONE: `TelegramTransport.is_authorized` now adapts the int id to the universal `telegram:<id>` form and delegates the membership check to `bot_core.is_authorized`. Behaviour is identical (verified: `_allowed_users` is already a `set[str]` of `telegram:<id>`; `None` → allow-all preserved). The transport-local copy is removed so the auth logic can't diverge._ — [src/gluon/transport/telegram.py:190](src/gluon/transport/telegram.py#L190)
...
- [x] **🔎 Confirm `events/subscribers.py` NotificationDispatcher has zero transports** — _CONFIRMED → RESOLVED (wire-or-remove done). `transport_dispatcher` (RUN_COMPLETED/FAILED/CANCELLED) was **removed**: it built a throwaway empty dispatcher and was redundant with the direct `notifier.notify()` calls the orchestrator/runner already make on the shared notifier. `question_escalator` (the *only* consumer of `question.escalated`, which the runner publishes over Redis specifically for the web process to fan out) was **wired** to the shared notifier: `register_subscribers(bus, store, notifier)` now threads it through, `create_app(notifier=…)` accepts it, and `serve` passes `bot_core.notifier` so web-submitted runs and escalations reach the same live Telegram/Discord transports. Transport-less deployments degrade to a graceful no-op._ — [src/gluon/events/subscribers.py:196](src/gluon/events/subscribers.py#L196)
  - **Done when:** the event-bus chat path either delivers via a real transport or is removed; question escalation reaches channels when transports are live. ✓
